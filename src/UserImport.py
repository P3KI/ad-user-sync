...
class UserImporter:
    ...
    def find_conflicting_user(self, domain :  adcontainer.ADContainer, account_name : str):
        q = adquery.ADQuery()
        q.execute_query(attributes=["distinguishedName"], where_clause="objectClass = 'user' AND sAMAccountName = '" + account_name + "'", base_dn=domain.dn)
        if len(q) == 0:
            return None
        else:
            dn = q.get_single_result()["distinguishedName"]
    ...
    def run(self):
        ...
        for user in users:
                ...
                try:
                    ...
                    if conflict_user:
                        ...
                        InteractiveImport.add_action(
                            InteractiveImport.UserResolveAccountNameConflict(cn, conflict_user.cn, account_name, attributes))
                        continue
                    else:
            ...
            if user["accountExpires"]:
                ...
                if (self.default_expiration is datetime) and (user_expiration > self.default_expiration): #Limit to self.default_expiration date
